...
Because of that if you use store.getState() (talking about Redux now) provided by useStore hook in react-redux package, you can see the difference, because when you call before, the getState() function you have the previous or initial environment (in its function body) and the second (notice that an await keyword in a async function is necessary, so that call getState once the state was updated) will have the new state, the new environment, note that I think the appropriate here would be create another useEffect where you put the state that you're concerned about in the dependecies array, to track it.
